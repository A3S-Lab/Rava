...
pub use rval::{JavaObject, ObjId, RVal};

// Thread-local output sink — set during tests to capture println output.
thread_local! {
    pub(crate) static OUTPUT: RefCell<Option<Vec<u8>>> = const { RefCell::new(None) };
    /// Carries the original thrown object across function call boundaries.
    pub(crate) static THROWN_OBJ: RefCell<Option<RVal>> = const { RefCell::new(None) };
