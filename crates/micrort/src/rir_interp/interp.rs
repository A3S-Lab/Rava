        ...
        match instr {
            ...
            RirInstr::ConstStr { ret, value } => {
                if let Some(rest) = value.strip_prefix("__try_catch__") {
                    ...
                    if let Some(val) = env.get(src_name).cloned() {
                        env.insert(ret.0.clone(), val.clone());
                        // Also propagate back so the src name is always findable
                        env.insert(src_name.to_string(), val);
                    }
                    // If src_name is not in env, don't update ret (keep its existing value).
                    // This handles the case where a variable wasn't modified in a loop body.
