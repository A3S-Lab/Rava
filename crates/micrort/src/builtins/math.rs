...
pub fn dispatch(func_id: u32, args: &[RVal]) -> Option<Result<RVal>> {
    ...
    match func_id {
        ...
        id if id == fnv("Math.pow") => {
            let b = args.get(1).map(|v| v.as_float()).unwrap_or(0.0);
            Some(Ok(RVal::Float(f1().powf(b))))
        }
        ...
        id if id == fnv("Math.atan2") => {
            let x = args.get(1).map(|v| v.as_float()).unwrap_or(0.0);
            Some(Ok(RVal::Float(f1().atan2(x))))
        }
        id if id == fnv("Math.hypot") => {
            let b = args.get(1).map(|v| v.as_float()).unwrap_or(0.0);
            Some(Ok(RVal::Float(f1().hypot(b))))
        }
        ...
        id if id == fnv("Math.floorDiv") => {
            let a = i1();
            let b = args.get(1).map(|v| v.as_int()).unwrap_or(1);
