...
pub fn dispatch(func_id: u32, args: &[RVal]) -> Option<Result<RVal>> {
    match func_id {
        id if id == fnv("Files.readString") || id == fnv("Files.readAllBytes") => {
            ...
            Some(std::fs::read_to_string(&path).map(RVal::Str).map_err(|e| {
                RavaError::JavaException {
        ...
        id if id == fnv("Files.readAllLines") => {
            ...
            match std::fs::read_to_string(&path) {
                ...
                Err(e) => Some(Err(RavaError::JavaException {
                    exception_type: "IOException".into(),
                    message: e.to_string(),
                })),
            }
        ...
        id if id == fnv("Files.writeString") || id == fnv("Files.write") => {
            ...
            Some(
                std::fs::write(&path, content)
                    .map(|_| RVal::Void)
                    .map_err(|e| RavaError::JavaException {
                        exception_type: "IOException".into(),
                        message: e.to_string(),
                    }),
            )
        }
        id if id == fnv("Files.exists") => Some(Ok(RVal::Bool(
            std::path::Path::new(&args.first().map(|v| v.to_display()).unwrap_or_default())
                .exists(),
        ))),
        id if id == fnv("Files.notExists") => Some(Ok(RVal::Bool(
            !std::path::Path::new(&args.first().map(|v| v.to_display()).unwrap_or_default())
                .exists(),
        ))),
        id if id == fnv("Files.isDirectory") => Some(Ok(RVal::Bool(
            std::path::Path::new(&args.first().map(|v| v.to_display()).unwrap_or_default())
                .is_dir(),
        ))),
        id if id == fnv("Files.isRegularFile") => Some(Ok(RVal::Bool(
            std::path::Path::new(&args.first().map(|v| v.to_display()).unwrap_or_default())
                .is_file(),
        ))),
        ...
        id if id == fnv("Files.createDirectory") || id == fnv("Files.createDirectories") => {
            let _ =
                std::fs::create_dir_all(args.first().map(|v| v.to_display()).unwrap_or_default());
            Some(Ok(RVal::Void))
        }
        id if id == fnv("Files.copy") => {
            ...
            Some(std::fs::copy(&src, &dst).map(|_| RVal::Void).map_err(|e| {
                RavaError::JavaException {
                    exception_type: "IOException".into(),
                    message: e.to_string(),
                }
            ...
            let entries: Vec<RVal> = std::fs::read_dir(&path)
                .map(|rd| {
                    rd.filter_map(|e| e.ok())
                        .map(|e| RVal::Str(e.path().to_string_lossy().into_owned()))
                        .collect()
                })
                .unwrap_or_default();
        ...
        id if id == fnv("File") || id == fnv("File.<init>") => Some(Ok(RVal::Str(
