...
pub fn dispatch(func_id: u32, args: &[RVal]) -> Option<Result<RVal>> {
    match func_id {
        ...
        id if id == fnv("AtomicInteger") || id == fnv("AtomicInteger.<init>")
            || id == fnv("AtomicLong") || id == fnv("AtomicLong.<init>") =>
        {
            Some(Ok(RVal::Int(args.first().map(|v| v.as_int()).unwrap_or(0))))
        }
        id if id == fnv("AtomicBoolean") || id == fnv("AtomicBoolean.<init>") => {
            Some(Ok(RVal::Bool(args.first().map(|v| v.is_truthy()).unwrap_or(false))))
        }
        ...
        id if id == fnv("ReentrantLock") || id == fnv("ReentrantLock.<init>")
            || id == fnv("ReentrantReadWriteLock") || id == fnv("ReentrantReadWriteLock.<init>")
            || id == fnv("CountDownLatch") || id == fnv("CountDownLatch.<init>")
            || id == fnv("Semaphore") || id == fnv("Semaphore.<init>")
            || id == fnv("CyclicBarrier") || id == fnv("CyclicBarrier.<init>") =>
        {
            Some(Ok(RVal::Null))
        }
...
pub fn dispatch_named(method: &str, args: &[RVal]) -> Option<Result<RVal>> {
    match method {
        ...
        "incrementAndGet"=> Some(Ok(RVal::Int(args.first().map(|v| v.as_int()).unwrap_or(0) + 1))),
        "decrementAndGet"=> Some(Ok(RVal::Int(args.first().map(|v| v.as_int()).unwrap_or(0) - 1))),
        "addAndGet"      => Some(Ok(RVal::Int(args.first().map(|v| v.as_int()).unwrap_or(0) + args.get(1).map(|v| v.as_int()).unwrap_or(0)))),
