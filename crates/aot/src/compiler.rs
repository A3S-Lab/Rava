    ...
    pub fn compile(&self, module: &mut Module, output: &Path) -> Result<()> {
        for pass in &self.passes {
            pass.run(module).map_err(|e| {
                RavaError::Codegen(format!("pass '{}' failed: {e}", pass.name()))
            })?;
        }
...
mod tests {
    ...
    fn passes_run_in_order() {
        ...
        assert_eq!(names, [
            "escape-analysis", "inlining", "dead-code-elim", "const-folding",
            "metadata-table-gen", "proxy-pregen", "micrort-bridge",
        ]);
    }
}
