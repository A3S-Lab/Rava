...
- **`rava run` / `rava test` → RIR interpreter** (`crates/micrort`). This is the **mature,
  supported path**: 413/413 end-to-end Java tests pass. Treat this as the product today.
...
- **Enums** are well-supported: `name()`/`ordinal()`/`values()`/`valueOf(name)`, `switch` on
  enums, custom methods, constructor fields, display via `println`/concat/`printf %s`, and
  `Arrays.toString(EnumType.values())` all work.
